...
function loadEventDataFromCSV() {
    return new Promise((resolve, reject) => {
        const results = [];
        fs.createReadStream('data/EventData.csv')
            .pipe(csv())
            .on('data', (data) => {
                results.push({
                    name: data.Name,
...
app.post('/api/user-lookup', (req, res) => {
...
async function startServer() {
    try {
        ...
        app.listen(PORT, () => {
            ...
            console.log(`👥 Guests loaded: ${eventData.length}`);
            console.log(`🧠 RAG Method: ${process.env.RAG_METHOD || 'basic'}`);
            console.log(`🤖 Model: ${process.env.OLLAMA_CHAT_MODEL || 'llama3.2:3b'}`);
            console.log(`🔮 HyDE: ${process.env.RAG_USE_HYDE === 'true' ? 'Enabled' : 'Disabled'}`);
            console.log(`🔍 Query Expansion: ${process.env.RAG_USE_QUERY_EXPANSION === 'true' ? 'Enabled' : 'Disabled'}`);
            console.log(`📊 Reranking: ${process.env.RAG_USE_RERANKING === 'true' ? 'Enabled' : 'Disabled'}`);
